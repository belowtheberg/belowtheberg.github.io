...
get(RSS_URL).then((data) => {
  ...
  for(let i = 0; i < data.items.length; i++) {
    ...
    let html = `<div class='episode col-4'><h4>${a.title}</h4><h5>${month(d.getMonth())} ${d.getDate()}, ${d.getFullYear()}</h5><p>${a.description}</p><a href='${a.link}'><button class='episode-listen'>Listen</button></a></div>`
    div.innerHTML += html;
  }
  console.log(data);
});
